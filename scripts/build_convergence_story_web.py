...
<main class="page">
  <header class="top">
    <div>
      ...
      <p>Documento técnico para explicar el proceso completo: qué se mantuvo fijo, qué se varió, cómo se ubicó la frontera de estabilidad y por qué se adoptó <code>dp=0.003 m</code> para las simulaciones productivas.</p>
    </div>
    <aside class="summary">
      <dl>
  ...
  <section>
    <h2>1. Objetivo del estudio de convergencia</h2>
    <p>El objetivo no fue ajustar una curva genérica ni buscar una única simulación “perfecta”. El objetivo fue decidir una malla de trabajo para la tesis y entender cómo la clasificación estable/falla cambia al modificar la resolución SPH.</p>
    <p>La resolución se controla con <code>dp</code>, el espaciamiento inicial entre partículas. La variable física usada para ubicar la frontera fue <code>μ</code>, el coeficiente de fricción bloque-suelo. Para cada malla se buscó el rango de <code>μ</code> donde el bloque pasa de falla a estable.</p>
    <p>Esto es importante: la convergencia se trató como <strong>sensibilidad de frontera</strong>, porque el problema no entrega una respuesta suave única, sino una decisión de umbral.</p>
  </section>
